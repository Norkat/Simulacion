...
Fichas = [(0, 0, 50), (0, 1, 1), (0, 2, 2), (0, 3, 3), (0, 4, 4), (0, 5, 5), (0, 6, 6), (1, 1, 41), (1, 2, 3), (1, 3, 4), (1, 4, 5), (1, 5, 6), (1, 6, 7), (2, 2, 42),
          (2, 3, 5), (2, 4, 6), (2, 5, 7), (2, 6, 8), (3, 3, 43), (3, 4, 7), (3, 5, 8), (3, 6, 9), (4, 4, 44), (4, 5, 9), (4, 6, 10), (5, 5, 45), (5, 6, 11), (6, 6, 46)]
...
def simular(num):
    ...
    if winner == 0:
        label = tk.Label(canvas, text="Gano el jugador 1",
                         font=("Chilanka", 20))
        canvas.create_window(350, 300, anchor='center', window=label)
        print("Gano el jugador 1")
    elif winner == 1:
        label = tk.Label(canvas, text="Gano el jugador 2",
                         font=("Chilanka", 20))
        canvas.create_window(350, 300, anchor='center', window=label)
        print("Gano el jugador 2")
    elif winner == 2:
        label = tk.Label(canvas, text="Gano el jugador 3",
                         font=("Chilanka", 20))
        canvas.create_window(350, 300, anchor='center', window=label)
        print("Gano el jugador 3")
    elif winner == 3:
        label = tk.Label(canvas, text="Gano el jugador 4",
                         font=("Chilanka", 20))
        canvas.create_window(350, 300, anchor='center', window=label)
...
def dar_ficha(n, x, y, xi, yi, player):
    ...
    if xi == 0:
        es = n * 5
        Y = yi * n
        canvas.create_image(x, y + Y + es, anchor="nw",
                            image=img_fichas[fichasP[player][n]])
        canvas.update()
        canvas.after(150, lambda: dar_ficha(n+1, x, y, xi, yi, player))
    else:
        es = n * 10
        X = xi * n
        canvas.create_image(x + X + es, y, anchor="nw",
                            image=img_fichas[fichasP[player][n]])
        canvas.update()
        canvas.after(150, lambda: dar_ficha(n+1, x, y, xi, yi, player))
...
menu_bar.add_command(label="Juegos", font=Font,
                     command=lambda: cambiar_panel("Juegos"))
...
opciones_menu.add_command(label="Nuevo", font=Font,
                          command=lambda: cambiar_panel("Inicio"))
opciones_menu.add_separator()
opciones_menu.add_command(label="Salir", font=Font, command=ventana.quit)
estadisticas_menu.add_command(
    label="Tabla", font=Font, command=lambda: cambiar_panel("Tabla"))
estadisticas_menu.add_command(
    label="Grafico", font=Font, command=lambda: cambiar_panel("Grafico"))

# Crear interfas de inicio

lb_pregunta = tk.Label(panel_Inicio, text="¿Cuantos juegos quieres simular?", font=(
    "Chilanka", 30), bg="cyan")
...
btn_simular = tk.Button(panel_Inicio, text="Simular", font=(
    "Chilanka", 20), command=lambda: comprobar_entrada(txt_numS.get()))
...
fondo = tk.PhotoImage(
    file="Python/Simulación/Domino/fondo.png")
...
# Crear una tabla
tabla = ttk.Treeview(panel_Tabla, columns=(
...
scrollbar_tabla = ttk.Scrollbar(
    panel_Tabla, orient=tk.VERTICAL, command=tabla.yview)
