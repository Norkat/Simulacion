...
def mostrarTabla(numerosRandom):
    ...
    labelTexto = Label(
        frameActual, text="Numeros Generados", font=("Arial", 25))
    labelTexto.pack()
    ...
    scrollbar = ttk.Scrollbar(tabla, orient=VERTICAL,
                              command=listaNumeros.yview)
    ...
    labelNumeros = Label(frameActual, text="Numeros Generados: " +
                         str(len(numerosRandom)), font=("Arial", 16))
    labelNumeros.pack()
    labelMedia = Label(
        frameActual, text="La media de los numeros es: " + str(media), font=("Arial", 16))
    labelMedia.pack()
    labelVarianza = Label(
        frameActual, text="La varianza de los numeros es: " + str(varianza), font=("Arial", 16))
    labelVarianza.pack()
...
def generarNumeros(cantidad):
    ...
    for i in range(cantidad):
        numerosRandom.append(random.randint(0, 10000))
...
def comprobarEntry():
    ...
    if dato == "" or not dato.isdecimal():
        ...
        if int(dato) > 10000:
            messagebox.showwarning("Advertencia", "Número fuera de rango")
        else:
...
def generarCantidad():
    generarNumeros(random.randint(10, 1000))
...
botonEnviar = Button(
    frameActual, text="Enviar entrada manual", command=comprobarEntry,)
botonEnviar.config(font=10, width=20, height=2)
botonEnviar.pack(pady=10)

botonRandom = Button(frameActual, text="Números Random",
                     command=generarCantidad)
